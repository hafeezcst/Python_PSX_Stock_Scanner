    ...
    try:
        for symbol in symbol_selection:
            ...
            if strong_buy_count >= 2 or strong_sell_count >= 2:
                recommendation = "Strong Buy" if strong_buy_count >= 2 else "Strong Sell"
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        print("Waiting for 5 minutes before starting the next analysis...")
        countdown = 120  # Set the countdown time in seconds
        while countdown > 0:
