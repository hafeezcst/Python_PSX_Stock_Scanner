...
def is_trading_hours():
    ...
    return (0 <= current_day <= 4) and (trading_start_time <= current_time <= trading_end_time)

def save_daily_report(data):
    today = datetime.now().strftime('%Y-%m-%d')
    filename = 'QSE-Daily_Analysis_Report.xlsx'
    try:
    ...
    try:
        if is_trading_hours():
            ...
            for symbol in symbol_selection:
                ...
                for time_frame in all_time_frames:
                    try:
                        ...
                        rsi_last = indicators['RSI[1]']  # Added to get the last RSI value
            ...
            if analysis_data:
                ...
                attachment_path = 'QSE-Daily_Analysis_Report.xlsx'
                subject = f"QSE-Daily Report - {datetime.now().strftime('%Y-%m-%d')}"
            ...
            # Sleep for 30 minute and then check again
            time.sleep(1800)  # 60 seconds = 1 minute
