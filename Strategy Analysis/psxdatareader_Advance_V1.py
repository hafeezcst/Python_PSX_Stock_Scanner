...
class DataReader:
    ...
    def simulate_trading_S4(self, data, investment=30000000, transaction_cost_per_share=0.00, buy_condition="row['RSI_14'] > 40"):
        """
    ...
    for symbol in symbols:
        ...
        try:
            ...
            for condition in buy_conditions:
                trading_log_S4 = data_reader.simulate_trading_S4(data, buy_condition=condition)
                data_reader.save_to_db(data, f'{symbol}_data')
                data_reader.save_to_db(trading_log_S4, f'{symbol}_trading_log')
                condition_name = condition.replace(" ", "_").replace("'", "").replace("[", "").replace("]", "").replace(">", "gt").replace("<", "lt")
                file_path = os.path.join(folder_path, f"{symbol}-{condition_name}.xlsx")
