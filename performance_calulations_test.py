...
def read_and_analyze_stock_data(file_path):
    try:
        ...
        if df[['Date and Time', 'Symbol', 'M Close']].isnull().any().any():
        ...
        df['Weekly Returns'] = df.groupby('Symbol')['M Close'].pct_change()
        logging.info("Calculated Weekly Returns for each symbol.")
        
        # Calculate cumulative returns for each symbol using transform
        df['Cumulative Returns'] = df.groupby('Symbol')['Weekly Returns'].transform(lambda x: (1 + x).cumprod())
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            analysis_df = df.groupby('Symbol').agg(
                ...
                First_Day_Price=('M Close', 'first'),
                Last_Day_Price=('M Close', 'last'),
...
# File paths
input_file_path = 'M-Advance_Technical_Analysis_KMI100_STRONG_BUY.xlsx'
output_file_path = 'M-KMI100_Performance_Analysis.xlsx'
