    ...
    try:
        ...
        if 'analysis' in locals() and analysis is not None and analysis.get_analysis() is not None:
            ...
            if summary is not None: 
                ...
                return symbol, summary, Close, Volume, RSI, RSI_Last, AO, Change, Charts, Financials, Technicals
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=500) as executor:  # Adjust max_workers as needed
        ...
        for future in futures:
            ...
            if result:
                symbol, summary, Close, Volume, RSI, RSI_Last, AO , Change ,base_url_charts,base_url_finan,base_url_tech = result
                print(f"{count}:  Symbol: {symbol}")
                summary= (summary.get('RECOMMENDATION'))
                print(summary)
                print(f"{analysis_type} CLOSE: {Close} Volume: {Volume} RSI: {RSI} LAST RSI: {RSI_Last} AO: {AO} %Change(D): {Change} ")
                print()
                count += 1

                # check and Append data to the list for CSV export if volme is greater than 5000
                if Volume > 5000:
                    analyzed_data.append([current_datetime, symbol, summary, Close, Volume, RSI, RSI_Last, AO, Change,base_url_charts,base_url_finan,base_url_tech])
                ...
                if summary == recommendation_filter and Volume > volume_theshold:
                    strong_buy_symbols.append([current_datetime, symbol, summary, Close, Volume, RSI, RSI_Last, AO, Change,base_url_charts,base_url_finan,base_url_tech])
        ...
        with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
            ...
            df_all = pd.DataFrame(analyzed_data, columns            =['Date and Time', 'Symbol', 'Summary', f'{analysis_type} Close', 'Volume', 'RSI', 'Last RSI', 'AO', '%Change(D)','Charts','Financials','Technicals'])
            df_strong_buy = pd.DataFrame(strong_buy_symbols, columns=['Date and Time', 'Symbol', 'Summary', f'{analysis_type} Close', 'Volume', 'RSI', 'Last RSI', 'AO', '%Change(D)','Charts','Financials','Technicals'])
