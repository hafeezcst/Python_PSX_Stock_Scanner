...
def main():
    ...
    if os.path.exists ( excel_file_path ) :
        ...
        while True:
            try:
                df_all_symbols = pd.read_excel ( excel_file_path, sheet_name='All Symbols' ).drop_duplicates(subset=['Symbol','Date and Time'],keep='last')
                df_strong_buy_symbols = pd.read_excel ( excel_file_path, sheet_name='Recommended_Symbols' ).drop_duplicates(subset=['Symbol','Date and Time'],keep='last')
                df_buy_symbols = pd.read_excel ( excel_file_path, sheet_name='Buy_Symbols' ).drop_duplicates(subset=['Symbol','Date and Time'],keep='last')
                df_sell_symbols = pd.read_excel ( excel_file_path, sheet_name='Sell_Symbols' ).drop_duplicates(subset=['Symbol','Date and Time'],keep='last')
    ...
    while True :
        ...
        body = today_Strong_buy.to_string ( index=False ,justify='left',col_space=10,header=True,na_rep='NaN',formatters=None,sparsify=None)
...
def main():
                ...
                if os.path.exists(excel_file_path):
                    with pd.ExcelFile(excel_file_path) as xls:
                        ...
                        if 'AO_Symbols' in xls.sheet_names:
                            df_AO_symbols = pd.read_excel(excel_file_path, sheet_name='AO_Symbols')
                        else:
    ...
    with pd.ExcelWriter(excel_file_path, engine='openpyxl',mode='a',if_sheet_exists='replace') as writer:
        ...
        column_J_ranges = [worksheet_all['J'], worksheet_strong_buy['J'], worksheet_buy['J'], worksheet_sell['J'], worksheet_AO['J']]
        column_L_ranges = [worksheet_all['L'], worksheet_strong_buy['L'], worksheet_buy['L'], worksheet_sell['L'], worksheet_AO['L']]
    ...
    while True :
        ...
        custom_message = f"Strong Buy Conditions: Volume >1M,,RSI>50, AO >0 and Strong_buy from Trading view.Suggested Shares are calculated based on {investment_amount} investment amount. "
